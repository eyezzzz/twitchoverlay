...
const TWITCH_SCOPES = ['user:read:chat', 'user:bot', 'channel:bot']
...
async function getAppAccessToken(env) {
  ...
  if (!response.ok) {
    throw new Error(`App token failed (${response.status}): ${await response.text()}`)
  }

  const payload = await response.json()
  if (!payload.access_token) throw new Error('Twitch did not return an app access token.')
...
async function createChatSubscription(env, config) {
  let appAccessToken
  try {
    appAccessToken = await getAppAccessToken(env)
  } catch (error) {
    return { ok: false, status: 502, payload: String(error?.message || error) }
  }
...
<body><div class="card"><div class="ok">TWITCH CONECTADA</div><h1>${safeName}</h1><p>O EventSub do chat foi configurado.</p><small>Envie uma mensagem no seu chat e ela aparecerá no painel LIVE CHAT do overlay.</small></div></body></html>`, {
...
function missingConfigResponse(env) {
  ...
  return new Response(`Missing runtime bindings: ${missing.join(', ') || 'unknown'}`, { status: 500 })
}

async function beginTwitchAuth(request, env) {
  if (!configReady(env)) return missingConfigResponse(env)
...
async function finishTwitchAuth(request, env) {
  ...
  const grantedScopes = new Set(tokens.scope || [])
  const missingScopes = TWITCH_SCOPES.filter((scope) => !grantedScopes.has(scope))
  if (missingScopes.length) {
    return new Response(`Twitch authorization is missing scopes: ${missingScopes.join(', ')}. Start again at /auth/twitch.`, { status: 400 })
  }
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/auth/twitch') return beginTwitchAuth(request, env)
    if (url.pathname === '/auth/twitch/callback') return finishTwitchAuth(request, env)
    if (url.pathname === '/api/twitch/eventsub' && request.method === 'POST') return handleTwitchEventSub(request, env, ctx)
    if (url.pathname === '/api/chat/messages') return room(env).fetch('https://chat-room/messages')
    if (url.pathname === '/api/chat/status') return room(env).fetch('https://chat-room/status')
    if (url.pathname === '/api/runtime/status') {
      return json({
        ...
        twitchRedirectUri: Boolean(env.TWITCH_REDIRECT_URI),
        chatRoom: Boolean(env.CHAT_ROOM),
      })
    }
